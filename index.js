...
const STAIR_HEIGHT = 100; // 블록 하나의 세로 높이 (px)
const STAIR_WIDTH = 100; // 블록 하나의 가로 너비 (px)
const LANE_WIDTH = 100; // 레인 하나의 가로 너비 (px), 블록 너비보다 약간 넓게 설정
...
function updateUI() {
    ...
    for(let i = start; i < end; i++) {
        ...
        stairEl.style.bottom = `${i * STAIR_HEIGHT}px`;
        stairEl.style.transform = `translateX(calc(-50% + ${getStairXOffset(stair.lane)}px))`;
    ...
    stairsContainer.style.height = `${(stairs.length + 5) * STAIR_HEIGHT}px`;
    stairsContainer.style.bottom = `calc(20vh - ${currentStairIndex * STAIR_HEIGHT}px)`;

    const currentCharacterStair = stairs[currentStairIndex];
    if (currentCharacterStair) {
        character.style.transform = `translateX(calc(-50% + ${getStairXOffset(currentCharacterStair.lane)}px))`;
